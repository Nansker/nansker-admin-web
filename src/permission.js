    ...
    if (to.path === '/login') {
      ...
      if (hasGetUserInfo) {
        ...
        try {
          ...
          if (store.getters.permissions.length < 1) {
            global.antRouter = []
            next()
          }
          const permissions = filterAsyncRouter(store.getters.permissions)// 1.过滤路由
          router.addRoutes(permissions) // 2.动态添加路由

          global.antRouter = permissions // 3.将路由数据传递给全局变量，做侧边栏菜单渲染工作
          next({
            ...to,
            replace: true
          })
